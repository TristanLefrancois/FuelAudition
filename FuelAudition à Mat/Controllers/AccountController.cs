    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user != null && !user.EmailConfirmed)
            {
                ...
                return RedirectToAction("EmailNotConfirm", "Account", new { email = user.Email });
            }
        ...
        [AllowAnonymous]
        public ActionResult EmailNotConfirm(string email, bool? send = false)
        {
            return View(new EmailNotConfirmViewModel { Email = email, Send = (bool)send });

        }

        [AllowAnonymous]
        [HttpPost]
        public ActionResult SendEmailAgain(string email)
        {
            ApplicationUser user = UserManager.FindByEmail(email);
            SendEmail(user);
            return RedirectToAction("EmailNotConfirm", "Account", new { email = user.Email, send = true });
        }
        ...
        private void SendEmail(ApplicationUser user)
        {
            ...
            html.Append("<table style=\"margin:0;padding:0;border-collapse:collapse;border: 5px solid black\">");
            html.Append("<tr style=\"border-bottom:5px solid black\">");
            html.Append("<td>");
            html.Append("<img src='http://testdev.fuelaudition.com/Content/Images/LogoFuelAudition.jpg' />");
            html.Append("</td>");
            html.Append("</tr>");

            html.Append("<tr style=\"background-color:#D32A2F;color:white;height:15px;font-size:16px\">");
            html.Append("<td style=\"padding: 0px 15px\">");
            html.Append("</td>");
            html.Append("</tr>");

            if (!String.IsNullOrEmpty(user.Nom) && !String.IsNullOrEmpty(user.Prenom))
            {
                html.Append("<tr style=\"background-color:#D32A2F;color:white;height:30px;font-size:16px\">");
                html.Append("<td style=\"padding: 0px 15px\">");
                html.Append("Bonjour " + user.Prenom + " " + user.Nom + ".");
                html.Append("</td>");
                html.Append("</tr>");
                html.Append("<tr style=\"background-color:#D32A2F;color:white;height:30px;font-size:16px\">");
                html.Append("<td style=\"padding: 0px 15px\">");
                html.Append("Bienvenue chez FuelAudition !");
                html.Append("</td>");
                html.Append("</tr>");
            }
            else
            {
                html.Append("<tr style=\"background-color:#D32A2F;color:white;height:30px;font-size:16px\">");
                html.Append("<td style=\"padding: 0px 15px\">");
                html.Append("Bonjour " + user.Email + ".");
                html.Append("</td>");
                html.Append("</tr>");
                html.Append("<tr style=\"background-color:#D32A2F;color:white;height:30px;font-size:16px\">");
                html.Append("<td style=\"padding: 0px 15px\">");
            ...
            html.Append("<tr style=\"background-color:#D32A2F;color:white;height:30px;font-size:16px\">");
            html.Append("<td style=\"padding: 0px 15px\">");
            html.Append("<a style=\"color:white \" href=\"" + url + Url.Action("ConfirmEmail", "Account") + "?userId=" + user.Id + "\" > Confirmez votre inscription</a>");
            html.Append("</td>");
            html.Append("</tr>");
            ...
            msg.From = new MailAddress("noreply@fuelaudition.com");
            msg.Subject = "Confirmation d'inscription";
            msg.IsBodyHtml = true;
            msg.Body = html.ToString();
            msg.To.Add(new MailAddress(user.Email));
            // msg.AlternateViews.Add(getEmbeddedImage("C:/TFS/Projet/FuelAudition/FuelAudition/Content/Images/LogoFuelAudition.jpg"));

            var smtp = new SmtpClient("mercury.namespro.ca", 2525);

            smtp.DeliveryMethod = SmtpDeliveryMethod.Network;
            smtp.UseDefaultCredentials = false;

            smtp.Credentials = new System.Net.NetworkCredential("noreply@fuelaudition.com", "Noreply!1234");
